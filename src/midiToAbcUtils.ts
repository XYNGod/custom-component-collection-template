...
const midiNoteToAbc = (noteNumber: number): string => {
...
const durationToAbc = (durationTicks: number, ticksPerBeat: number): string | number => {
...
export const convertMidiToAbc = async (file: File, addStep: (step: string) => void): Promise<string> => {
  try {
